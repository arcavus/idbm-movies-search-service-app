package com.arcavus.armada.movies.service.links;

import com.arcavus.armada.movies.dto.links.FindAcquaintanceLinksResult;
import com.arcavus.armada.movies.dto.links.Link;
import com.arcavus.armada.movies.repository.AcquaintanceLinksRepository;
...
public class AcquaintanceLinksService {
            ...
            for (Name sourceName : sourceNames) {
                for (Name targetName : targetNames) {
                    ...
                    try (Transaction tx = session.beginTransaction()) {
                        Boolean allNodesExist = acquaintanceLinksRepository.checkIfNameNodesExist(tx, sourceNameNconst, targetNameNconst);
                        if (!allNodesExist) {
                            findAcquaintanceLinksResultBuilder.resultStatus(GET_LINKS_OPERATION_NOT_OK_MSG_1);
                        } else {
                            Try<Path> pathOperationResult = acquaintanceLinksRepository.getPath(sourceNameNconst, targetNameNconst, tx);
                            pathOperationResult.onSuccess(path -> {
    ...
    public void prepareNeo4jDbFromMySql() {
        try (Driver driver = getDriver();
            ...
            log.log(Level.INFO,"AcquaintanceLinksService#prepareNeo4jDbFromMySql, total steps to execute: " + steps);
            int offset = 0;
            long startTimeInNs = System.nanoTime();
            for (int i = 0; i < steps; i++, offset += PROCESS_BATCH_SIZE) {
                log.log(Level.INFO,"AcquaintanceLinksService#prepareNeo4jDbFromMySql, currently executing step: " + (i + 1) + "/" + steps);
            ...
            log.log(Level.INFO,"AcquaintanceLinksService#prepareNeo4jDbFromMySql, total time to prepare Neo4J in minutes: " + totalTimeInMinutes);
        }
    }
